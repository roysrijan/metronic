                ...
                @foreach(theme()->getOption('product', 'demos') as $id => $demo)
                    <!--begin::Demo-->
                        <div class="mb-7">
                            ...
                            <div class="overlay overflow-hidden border border-4 <?php util()->putIf($id === theme()->getDemo(), 'border-primary', '')?> p-2 rounded">
                                @if($demo['published'] === true)
                                    <div class="overlay-wrapper rounded">
                                        <img src="{{ theme()->getMediaUrlPath() . $demo["thumbnail"] }}" alt="demo" class="w-100 rounded"/>
                                    </div>

                                    <div class="overlay-layer bg-dark bg-opacity-10">
                                        <a href="{{ theme()->getOption('product', 'preview') . '/' . $id }}" class="btn btn-primary shadow">Preview</a>
                                    </div>
                                @else
                                    <div class="overlay-wrapper">
                                        <img src="{{ theme()->getMediaUrlPath() . $demo["thumbnail"] }}" alt="demo" class="w-100 rounded opacity-75"/>
                                    </div>
